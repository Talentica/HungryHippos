...
sshpass -p $node_pwd ssh root@$1 "sh -c 'cd hungryhippos;nohup java -cp node.jar:test-jobs.jar com.talentica.hungryHippos.node.NodeStarter ./config.properties > /dev/null 2>&1 &'"
