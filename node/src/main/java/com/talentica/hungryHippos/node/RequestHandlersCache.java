...
public enum RequestHandlersCache {
    ...
    public RequestHandlingTool get(int nodeId, int fileId){
        Map<Integer,RequestHandlingTool> fileIdToRequestHandlerMap= nodeIdToFileIdToRequestHandlersMap.get(nodeId);
        ...
        if(fileIdToRequestHandlerMap.get(fileId)==null){
            try {
                ...
                String hhPath ="";
                RequestHandlingTool requestHandlingTool = new RequestHandlingTool(fileId,hhPath,nodeId+"");
                fileIdToRequestHandlerMap.put(fileId,requestHandlingTool);
            } catch (IOException | InterruptedException |
                    ClassNotFoundException | KeeperException | JAXBException e) {
                e.printStackTrace();
            }
        }
