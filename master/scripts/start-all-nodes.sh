...
do
	...
	sshpass -p $node_pwd ssh root@$node "cd hungryhippos;nohup java -cp node.jar:test-jobs.jar com.talentica.hungryHippos.manager.node.NodeStarter ./config.properties &"
done
