...
public class Sharding {
	...
	private final static String BAD_RECORDS_FILE = CoordinationApplicationContext.getZkCoordinationConfigCache()
			.getCommonConfig().getBadRecordsFileOut() + "_sharding.err";
	...
	public void dumpShardingTableFiles(String directoryPath) {
		CommonUtil.dumpFileOnDisk(Sharding.bucketCombinationToNodeNumbersMapFile, bucketCombinationToNodeNumbersMap,
				directoryPath);
		CommonUtil.dumpFileOnDisk(Sharding.bucketToNodeNumberMapFile, bucketToNodeNumberMap, directoryPath);
		CommonUtil.dumpFileOnDisk(Sharding.keyToValueToBucketMapFile, keyToValueToBucketMap, directoryPath);
	}

	private void updateBucketToNodeNumbersMap(Reader data)
			throws IOException, ClassNotFoundException, KeeperException, InterruptedException, JAXBException {
	...
	public Map<String, List<KeyValueFrequency>> getSortedKeyToListOfKeyValueFrequenciesMap()
			throws ClassNotFoundException, FileNotFoundException, KeeperException, InterruptedException, IOException,
			JAXBException {
