...
public class DataRetrievalThread extends Thread {
    ...
    public void run() {
        try {
            ...
            if (dimensionOperand > -1) {
                handleShardedFileDownload(hungryHippoFilePath, dimensionOperand, nodeId, offset, inputBuffer);
            }else{
    ...
    private void handleFileDownload(String hungryHippoFilePath, long offset, byte[] inputBuffer) throws IOException {
        int len;RandomAccessFile raf = new RandomAccessFile(rootDirectory+hungryHippoFilePath, "r");
    ...
    private void handleShardedFileDownload(String hungryHippoFilePath, int dimensionOperand, int nodeId, long offset, byte[] inputBuffer) throws IOException {
        int len;
        ...
        for (String dataFolder : dataFolderSet) {
            ...
            if ((dataFolderIntVal & dimensionOperand) == dimensionOperand) {
                for (String dataFileName : nodeIdSet) {
                    File dataFile = new File(rootDirectory + hungryHippoFilePath + File.separatorChar + dataFilePrefix + dataFolder + File.separatorChar + dataFileName);
                    long dataFileSize = dataFile.length();
                    if(dataFileSize>0){
                        if (offset == 0) {
                            BufferedInputStream bis = new BufferedInputStream(new FileInputStream(dataFile.getAbsolutePath()));
