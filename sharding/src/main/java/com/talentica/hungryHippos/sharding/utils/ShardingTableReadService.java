		...
		try {
				...
				if(children != null){
					for(String child : children){
						String keyToBucketMapPath = bucketCombinationPath + File.separatorChar 
								+ child + File.separatorChar + ZkNodeName.KEY_TO_BUCKET.getName();
						...
						for(String keyToBucketChild : keyToBucketChildren){
							...
							/*int bucketId = 0;
							long bucketSize = 0;
							for(String bucketInfo : bucketChildren){
								String [] info = bucketInfo.split(ZkNodeName.EQUAL.getName());
						...
						String nodesPath = bucketCombinationPath + File.separatorChar 
								+ child + File.separatorChar + ZkNodeName.NODES.getName();
						...
						for(String nodeChild : nodesChildren){
							...
							for(String nodeInfo : nodeChildren){
								String [] info = nodeInfo.split(ZkNodeName.EQUAL.getName());
