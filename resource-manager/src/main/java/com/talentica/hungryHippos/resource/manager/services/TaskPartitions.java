...
/**
 * {@code TaskPartitions} do the greedy selections of the task to make it available to the run based
...
public final class TaskPartitions {
  ...
  /**
   * creates an instance of TaskPartitions.
   * 
  ...
  /**
   * retrieves the list of resource consumers associated with resourced id.
   * 
   * @return
   */
  public Map<Integer, List<ResourceConsumer>> getIterationWiseResourceConsumers() {
    ...
    TreeMap<Integer, List<ResourceConsumer>> resourcesPartition =
        new TreeMap<Integer, List<ResourceConsumer>>();
    ...
    if (outBoundResources != null && !outBoundResources.isEmpty()) {
      LOGGER.info("NUMBER OF CONSUMERS HAVING MEMORY SIZE BEYOND AVAILABLE MEMORY {}",
          outBoundResources.size());
    }
    LOGGER.info("ELAPSED TIME IN TASK PARTITIONING {} ms",
        (System.currentTimeMillis() - startTime));
    return resourcesPartition;
  }

  /**
   * retrieves the {@link ResourceConsumer} }
   * 
   * @return
   */
  ...
  /**
   * sets the outbound resource consumers
   * 
   * @param outBoundResources
   */
  ...
  /**
   * retrieves the available memory of ram.
   * 
   * @return
   */
