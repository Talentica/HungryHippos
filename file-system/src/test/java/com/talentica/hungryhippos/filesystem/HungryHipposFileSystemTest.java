...
public class HungryHipposFileSystemTest {
  ...
  private HungryHipposFileSystem system = HungryHipposFileSystem.getInstance();
  ...
  public void testCreateZnode() {
    ...
    String path = system.createZnode(testFolder);
    assertNotNull(path);
  }
  ...
  public void testCreateZnodeArg2() {
    ...
    String path = system.createZnode(testFolder, signal);
    try {
      signal.await();
  ...
  public void testCreateZnodeArg3() {
    ...
    String path = system.createZnode(testFolder, "testing");
    assertNotNull(path);
  }

  @Test
  public void testCheckZnodeExists() {
    boolean flag = system.checkZnodeExists("test2");
    assertTrue(flag);
  }

  @Test
  public void testCheckZnodeExistsNegative() {
    boolean flag = system.checkZnodeExists("test3");
    assertFalse(flag);
  }

  @Test
  public void testSetData() {
    system.setData("test1", "data Passed");

  }

  @Test
  public void testGetData() {
    String s = system.getData("test1");
    assertNotNull(s);
  }
...
//      PowerMockito.when(CoordinationApplicationContext.getZkProperty()).thenReturn(zkProperty);
      ...
      Mockito.doNothing().when(nodesManager).createPersistentNode(Mockito.anyString(),
          new CountDownLatch(1), Mockito.eq(Mockito.any()));
      Mockito.when(zkProperty.getValueByKey(FileSystemConstants.ROOT_NODE))
          .thenReturn(fileSystemRootNodeZKPath);

      system.updateFSBlockMetaData(fileZKNode, nodeIp, dataFileZKNode, datafileSize);
