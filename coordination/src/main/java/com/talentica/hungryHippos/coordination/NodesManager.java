...
public class NodesManager implements Watcher {
  ...
  public void startup() throws Exception {
    ...
    if (registrationListener != null) {
      LOGGER.info("Nodes Manager Started successfully.. NOW ,There are currently {} "
          + "servers: {}", registrationListener.getRegisteredServers().size(), registrationListener
          .getRegisteredServers().toString());
    }
    getSignal.await();
  }
  ...
  public void defaultNodesOnStart() throws IOException {
    ...
    createPersistentNode(
        PathUtil.SEPARATOR_CHAR + zkConfiguration.getPathMap().get(PathEnum.NAMESPACE.name()), null);
  ...
  private void createNode(final String node, CountDownLatch signal, CreateMode createMode,
    ...
    zk.create(node, (data != null && data.length != 0) ? ZkUtils.serialize(data[0]) : null,
        ZooDefs.Ids.OPEN_ACL_UNSAFE, createMode, new AsyncCallback.StringCallback() {
          ...
          private void createParentNode(final String node) throws InterruptedException,
              KeeperException {
    ...
    switch (watchedEvent.getType()) {
      ...
      case NodeDataChanged:
        ...
        try {
          ...
          LOGGER.info(
              "There was an error while updating the data " + "associated with node {}: {}",
  ...
  synchronized public Status registerAlert(Server server, boolean persistent) {
    ...
    try {
      ...
      String msg =
          String.format("Caught an exception while trying to alert {} ({})", server,
  ...
  protected String buildMonitorPathForServer(String serverHostname) {
    ...
    } else if (serverHostname.startsWith(zkConfiguration.getPathMap()
        .get(PathEnum.ALERTPATH.name()))) {
  ...
  public String buildConfigPath(String fileName) {
    String buildPath =
        zkConfiguration.getPathMap().get(PathEnum.CONFIGPATH.name()) + PathUtil.SEPARATOR_CHAR
            + fileName;
    return buildPath;
  }
   ...
   * @throws IOException
   */
  public Object getConfigFileFromZNode(String fileName) throws KeeperException,
  ...
  public Object getObjectFromZKNode(String nodePath) throws KeeperException, InterruptedException,
      ClassNotFoundException, IOException {
    Stat stat = null;
    ...
    try {
      for (int nodeId = 0; nodeId < this.servers.size(); nodeId++) {
        String pushNotification =
            pathMap.get(PathEnum.BASEPATH.name()) + PathUtil.SEPARATOR_CHAR
