...
/**
 * Created by debasishc on 1/9/15. Will see to it later
 */
@Deprecated
public class MarshalingClassGenerator {
  ...
  public static <E> Class<? extends E> generateMarshalingImplementation(Class<E> interfaceClass,
    ...
    FieldNode byteBufferNode = new FieldNode(Opcodes.V1_8, Opcodes.ACC_PUBLIC, "byteBuffer",
        Type.getInternalName(ByteBuffer.class), Type.getDescriptor(ByteBuffer.class), null);
    ...
    constructor.instructions
        .add(new TypeInsnNode(Opcodes.NEW, Type.getInternalName(ByteBuffer.class)));
    ...
    for (Method m : getterMethods) {
      MethodNode method = new MethodNode(Opcodes.ACC_PUBLIC, m.getName(),
          "()" + Type.getDescriptor(m.getReturnType()), null, null);
      int size = sizeMap.get(m.getReturnType());
      for (int i = 0; i < size; i++) {
        method.instructions.add(new FieldInsnNode(Opcodes.GETFIELD, generatedClassName,
            "byteBuffer", Type.getDescriptor(ByteBuffer.class)));
