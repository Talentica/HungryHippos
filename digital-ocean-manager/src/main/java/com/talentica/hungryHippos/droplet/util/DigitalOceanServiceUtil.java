...
public class DigitalOceanServiceUtil {
  private static final Logger LOGGER = LoggerFactory.getLogger(DigitalOceanServiceUtil.class);
  ...
  public static void performServices(DigitalOceanServiceImpl dropletService,
    ...
    switch (dropletEntity.getRequest()) {
      case CREATE:
        ...
        Droplets retDroplest = createDroplets(dropletService, dropletEntity, dropletNamePattern);
        if (jobUUId != null && jobUUId.length == 1)
      ...
      case DELETE:
        for (String dpltId : getDropletIdsFile(jobUUId[0])) {
          ...
          try {
            ...
            LOGGER.info("Destroy of droplet id {} is initiated.", dpltId);
          } catch (DigitalOceanException ex) {
            LOGGER.info("Unable to delete the droplet id {} and exception {}", dpltId, ex);
          }
        }
        break;
      ...
      case RENAME:
        ...
        if (dropletIds == null)
          return;
        dropletService.renameDroplet(Integer.valueOf(dropletIds[0]), dropletEntity.getDroplet()
            .getName());
        LOGGER.info("Droplet id {} is renamed to {}", dropletEntity.getDroplet().getId(),
      ...
      case GET_ALL_DROPLET_INFO:
        droplets =
            dropletService.getAvailableDroplets(dropletEntity.getPageNo(),
                dropletEntity.getPerPage());
        List<Integer> dropletIdList = new ArrayList<>();
        for (Droplet dropletObj : droplets.getDroplets()) {
          if (!dropletObj.getName().contains(dropletNamePattern))
            continue;
          String[] nameArray = dropletObj.getName().split("-");
          if (nameArray.length < 2)
            continue;
          if (!dropletNamePattern.equalsIgnoreCase(nameArray[1]))
            continue;
          dropletIdList.add(dropletObj.getId());
          LOGGER.info("Droplet id {} , name {} , ip {}", dropletObj.getId(), dropletObj.getName(),
              dropletObj.getNetworks().getVersion4Networks().toString());
        }
      ...
      case GET_ALL_PROPERTIES_OF_DIGITAL_OCEAN:
        ...
        images =
            dropletService
                .getAvailableImages(dropletEntity.getPageNo(), dropletEntity.getPerPage());
        LOGGER.info("All images available {}", images.toString());
        Keys keys = dropletService.getAvailableKeys(dropletEntity.getPageNo());
        LOGGER.info("All Keys available {}", keys.toString());
        Regions regions = dropletService.getAvailableRegions(dropletEntity.getPageNo());
        LOGGER.info("All regions available {}", regions.toString());
        Sizes sizes = dropletService.getAvailableSizes(dropletEntity.getPageNo());
      ...
      case SNAPSHOT:
        ...
        if (dropletEntity.getSnapshotName() == null
          ...
          dropletService.takeDropletSnapshot(dropletId, dropletEntity.getSnapshotName());
        }
   ...
   */
  private static void populatePresetValues(DigitalOceanServiceImpl dropletService,
      DigitalOceanEntity dropletEntity, Image image) throws DigitalOceanException,
      RequestUnsuccessfulException {
    Images images;
    if (dropletEntity.getDroplet().getImage().getId() != null) {
      image = dropletService.getImageInfo(dropletEntity.getDroplet().getImage().getId());
      ...
      for (int page = 1;; page++) {
        ...
        for (Image img : images.getImages()) {
          if (dropletEntity.getDroplet().getImage().getName().equalsIgnoreCase(img.getName())) {
            LOGGER.info("Image id {} and name {}", img.getId(), img.getName());
            image = img;
            isImageFound = true;
            break;
          }
   ...
   * @throws RequestUnsuccessfulException
   */
  private static void getAndPopulateAllSshKeys(DigitalOceanServiceImpl dropletService,
      DigitalOceanEntity dropletEntity) throws DigitalOceanException, RequestUnsuccessfulException {
    List<Key> newKeys = new ArrayList<Key>();
    Keys fetchKeys = dropletService.getAvailableKeys(null);
    if (fetchKeys != null && fetchKeys.getKeys() != null && !fetchKeys.getKeys().isEmpty()) {
   ...
   * @throws RequestUnsuccessfulException
   */
  private static Droplets createDroplets(DigitalOceanServiceImpl dropletService,
      DigitalOceanEntity dropletEntity, String dropletNamePattern) throws DigitalOceanException,
      RequestUnsuccessfulException {
    Droplets droplets;
    ...
    for (int index = 0; index < onOfDroplets; index++) {
      dropletEntity.getDroplet().getNames()
          .add(PRIFIX + HYPHEN + dropletNamePattern + HYPHEN + index);
      if (dropletEntity.getDroplet().getNames().size() == MAXIMUM_DROPLETS_IN_BATCH) {
        droplets = dropletService.createDroplets(dropletEntity.getDroplet());
        dropletList.addAll(droplets.getDroplets());
        dropletEntity.getDroplet().getNames().clear();
      }
    }

    if (dropletEntity.getDroplet().getNames().size() != 0
        && dropletEntity.getDroplet().getNames().size() < 10) {
      droplets = dropletService.createDroplets(dropletEntity.getDroplet());
      dropletList.addAll(droplets.getDroplets());
      dropletEntity.getDroplet().getNames().clear();
    }
    retDroplest.setDroplets(dropletList);
    LOGGER.info("Droplet/Droplets is/are of id/ids {} is initiated", retDroplest.toString());
    return retDroplest;
  }
   ...
   * @throws Exception
   */
  private static void performConfigurationService(DigitalOceanServiceImpl dropletService,
    ...
    if (formatFlag.equals("Y")) {
      ...
      while (!zkStatus.toLowerCase().contains("Mode".toLowerCase())) {
        ...
        if (retryCounter == 0) {
          LOGGER
              .info("Unable to start the zookeeper server. Now copying the logs file to ngnix server.");
          /*
   ...
   * @throws RequestUnsuccessfulException
   */
  private static List<Droplet> getActiveDroplets(DigitalOceanServiceImpl dropletService,
      Droplets droplets, String jobuuid) throws InterruptedException, DigitalOceanException,
      RequestUnsuccessfulException {
   ...
   * @throws IOException
   */
  private static List<String> generateServerConfigFile(List<Droplet> droplets, String jobUUId)
      throws IOException {
    int index = 0;
   ...
   * @return
   */
  private static String[] validateForSingalIdRequest(DigitalOceanEntity dropletEntity) {
