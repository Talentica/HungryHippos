...
import com.talentica.spark.job.executor.SumTextExecutor;
 ...
 *
 */
public class SumTextJob implements Serializable {
  ...
  public static void main(String[] args) throws FileNotFoundException, JAXBException,
    ...
    for (Job job : getSumJobMatrix().getJobs()) {
      ...
      JavaRDD<Tuple2<String, Double>> resultRDD =
          SumTextExecutor.process(hipposRDD, descriptionBroadcast, jobBroadcast);
  ...
  private static void initializeSparkContext(String masterIp, String appName) {
    if (SumTextJob.context == null) {
      SparkConf conf = new SparkConf().setMaster(masterIp).setAppName(appName);
      try {
        SumTextJob.context = new JavaSparkContext(conf);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
