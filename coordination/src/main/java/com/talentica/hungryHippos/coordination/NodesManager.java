...
public class NodesManager implements Watcher {
	...
	public NodesManager(String zookeeperConfigPath) {
		...
		zkConfiguration = new ZookeeperConfiguration(pathMap, Integer.valueOf(zkDeafultSetting.getSessionTimout()));
	}
	...
	/**
	 * To save the object which contains only the primitive data type as
	 * instance variable. The hierarchy is managed by the parent node followed
	 * by the instance variable and it corresponding value i.e (id=0) or (size =
	 * 1) as complete node name.
	 * 
	 * @param parentNode
	 *            is the node which is basically contains all the values of the
	 *            object as children of it.
	 * @param signal
	 *            is to ensure that all the relevant children should be created
	 *            as node.
	 * @param object
	 *            is to save onto zookeeper as a part of node formation.
	 * @throws IOException
	...
	public void createPersistentNodeSync(final List<String> paths) throws IOException, InterruptedException {
		int length = paths.size();
		CountDownLatch counter = new CountDownLatch(length);
	...
	StringCallback createNodeCallBack = new StringCallback() {
		...
		private void createNode(final String node, boolean makeLastNode) throws KeeperException, InterruptedException {
	 ...
	 * @throws IOException
	 */
	public void createPersistentNode(final String node, CountDownLatch signal, boolean isTransient) throws IOException {
		if (isTransient) {
			if (signal != null) {
				signal.countDown();
			}
	...
	private void createServersMap() {
		...
		List<Node> nodes;
		try {
			nodes = CoordinationApplicationContext.getServers();
		} catch (ClassNotFoundException | KeeperException | InterruptedException | IOException | JAXBException e) {
			LOGGER.info("Unable to fetch the server configuration from cluster configuration.");
			return;
		}
