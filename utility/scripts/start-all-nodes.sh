...
ssh -o StrictHostKeyChecking=no root@$node "sh -c 'cd hungryhippos/node;java -cp node.jar:test-jobs.jar com.talentica.hungryHippos.node.DataReceiver $jobUuid > ./system.out 2>./system.err &'"
done
