...
public class DataRowProcessor implements RowProcessor {
  ...
  private void removeValuesToFreeupMemory() {
    ...
    long memoryNeeded =
        (MemoryStatus.getMaxMemory() - MINIMUM_FREE_MEMORY_REQUIRED_TO_BE_AVAILABLE_IN_MBS);
    long memoryDeficiency = usedMemory - memoryNeeded;
    long numberOfValuesToBeDeleted =
  ...
  private boolean isNewValueSetWhichCanBeProcessed(ValueSet valueSet) {
    return processedTillValueSetInLastBatches == null
        || valueSet.compareTo(processedTillValueSetInLastBatches) > 0;
  }
