...
public class NodeDataStoreIdCalculatorTest {
	...
	public void setup() throws Exception {
		keyToValueToBucketMap = (Map<String, Map<Object, Bucket<KeyValueFrequency>>>) new ObjectInputStream(
				this.getClass().getResourceAsStream("/keyToValueToBucketMap")).readObject();
		bucketToNodeNumberMap = (Map<String, Map<Bucket<KeyValueFrequency>, Node>>) new ObjectInputStream(
				this.getClass().getResourceAsStream("/bucketToNodeNumberMap")).readObject();
		FieldTypeArrayDataDescription dataDescription = getDataDescription();
		noOfBytesInOneDataSet = dataDescription.getSize();
		nodeDataStoreIdCalculator = new NodeDataStoreIdCalculator(keyToValueToBucketMap, bucketToNodeNumberMap, 1,
				dataDescription);
	}

	@Test
	public void testStoreId() throws IOException {
		File dataFile = new File(this.getClass().getResource("/nodeDatatoreIdCalcSampleData.txt").getPath());
