...
/**
 * This is a class for uploading the filesystem configuration xml in the zookeeper
...
public class FileSystemStarter {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (configuration.getServerPort() == 0 || configuration.getFileStreamBufferSize() <=0 || configuration.getQueryRetryInterval()<=0
                    ||configuration.getMaxQueryAttempts()<=0||configuration.getMaxClientRequests() <=0
                    || configuration.getDataFilePrefix() == null ||configuration.getRootDirectory() == null
                    ||"".equals(configuration.getDataFilePrefix())||"".equals(configuration.getRootDirectory())) {
                throw new RuntimeException(
                        "Invalid configuration file or cluster configuration missing." + fileSystemConfigFilePath);
            }
            FileSystemContext.uploadFileSystemConfig(
            ...
            LOGGER.error("Uploading FileSystem configuration", exception);
