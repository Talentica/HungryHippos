...
public class Sharding {
    ...
    private NodeCombinationArray nodeCombinationArray = new NodeCombinationArray();
        ...
        try {
            ...
            if (LOGGER.isDebugEnabled()) {
                ...
                LOGGER.debug("bucketCombinationToNodeNumbersMap: "
                        + sharding.nodeCombinationArray.toString());
            }
            try {
    ...
    public void shardAllKeys() throws NodeOverflowException {
        ...
        String[] keys = Property.getShardingDimensions();
        long maxIndex = getMaxIndex(Arrays.asList(keys)) + 1;
        NodeCombination[] nodeCombinations = new NodeCombination[(int) maxIndex];
        nodeCombinationArray.setNodeCombinations(nodeCombinations);
    ...
    private void makeShardingTable(BucketCombination source, List<String> keyNames) throws NodeOverflowException {
        ...
        if (keyName == null) {
            ...
            int index = calculatedIndex(source.getBucketsCombination().entrySet());
            NodeCombination nodeCombination = new NodeCombination();
            nodeCombination.setNodes(nodesForBucketCombination);
            nodeCombination.setIndex(index);
            nodeCombinationArray.getNodeCombinations()[index] = nodeCombination;
        } else {
    ...
    private int calculatedIndex(Set<Map.Entry<String, Bucket<KeyValueFrequency>>> buckets) {
        int b = 1;
        int result = 0;
        for (Map.Entry<String, Bucket<KeyValueFrequency>> bucket : buckets) {
            Collection<Bucket<KeyValueFrequency>> keyBuckets = (Collection<Bucket<KeyValueFrequency>>) keyToValueToBucketMap.get(bucket.getKey()).values();
            ...
            b *= bucket.getValue().getId();
        }
        return result;
    }

    private int getMaxIndex(List<String> keyNames) {
        int b = 1;
        int result = 0;
        if (keyNames != null && !keyNames.isEmpty()) {
            for (String key : keyNames) {
                result += b * keyToValueToBucketMap.get(key).size();

                Collection<Bucket<KeyValueFrequency>> keyBuckets = (Collection<Bucket<KeyValueFrequency>>) keyToValueToBucketMap.get(key).values();
                List<Bucket<KeyValueFrequency>> keyBucketList = new ArrayList(keyBuckets);
                Collections.sort(keyBucketList);
                b *= keyBucketList.get(keyBucketList.size() - 1).getId();
            }
        }
        return result;
    }
