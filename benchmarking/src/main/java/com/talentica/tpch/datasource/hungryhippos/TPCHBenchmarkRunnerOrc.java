...
import com.talentica.hungryHippos.rdd.HHFileSystem;
 ...
 */
public class TPCHBenchmarkRunnerOrc {
    private static HHSparkContext context;
    private static SparkSession sparkSession;
    protected static Logger LOGGER = LoggerFactory.getLogger(TPCHBenchmarkRunnerOrc.class);
    ...
    public static void main(String[] args) throws JAXBException, IOException, InterruptedException {
        ...
        String masterIp = args[3];
        initSparkContext(appName, clientConfigPath,masterIp);

        SQLContext sqlContext = new SQLContext(context);
        Dataset<Row> df ;
        df = sqlContext.read().format("com.talentica.hungryhippos.datasource.orc").
                option("dimension","0").
                load(hhDataFolderPath + "CUSTOMER");
        df.createOrReplaceTempView("CUSTOMER");
        df = sqlContext.read().
                format("com.talentica.hungryhippos.datasource.orc").
                option("dimension","0").
    ...
    protected static void initSparkContext(String appName,
                                           String clientConfigPath,String masterIp) {
        try {
            SparkConf sparkConf = new SparkConf().setAppName(appName).setMaster(masterIp);
            SparkContext sparkContext = new SparkContext(sparkConf);
            sparkSession = new SQLContext(sparkContext).sparkSession();
            context = new HHSparkContext(sparkSession.sparkContext(), clientConfigPath);
            sparkSession.sparkContext().conf().set("fs.hhfs.impl","com.talentica.hungryHippos.rdd.HHFileSystem");
            sparkSession.sparkContext().conf().set(HHFileSystem.connectString, context.connectString());
        } catch (Exception e) {
            e.printStackTrace();
        }
