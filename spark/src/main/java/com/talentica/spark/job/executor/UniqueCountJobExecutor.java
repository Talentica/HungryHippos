...
public class UniqueCountJobExecutor implements Serializable {
	...
	public void startUniqueCountJob(JavaSparkContext context, HHRDDConfiguration hhrddConfiguration,
		...
		JavaPairRDD<String, Integer> javaPairRDD = hipposRDD.toJavaRDD()
				.mapToPair(new PairFunction<byte[], String, Integer>() {
					...
					public Tuple2<String, Integer> call(byte[] buf) throws Exception {
						...
						for (int index = 0; index < dataDes.getValue().getNumberOfDataFields(); index++) {
						...
						Integer value = Integer
								.valueOf(readerVar.readAtColumn(jobBroadcast.value().getCalculationIndex()).toString());
						return new Tuple2<String, Integer>(key, value);
					}
				});
		HHJavaRDD<Tuple2<String, Long>> hhJavaRDD = (HHJavaRDD<Tuple2<String, Long>>) javaPairRDD
		...
		String path = hhrddConfiguration.getOutputFile() + jobBroadcast.value().getJobId();
		hhJavaRDD.saveAsTextFile(path);
		
		//HHRDDFileUtils.saveAsText(javaRDD, path);
