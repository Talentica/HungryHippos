...
public class FloatingDecimal {
  ...
  private void dtoa(int binExp, long fractBits, int nSignificantBits) {
    ...
    decExp = (int) Math
        .floor((d2 - 1.5D) * 0.289529654D + 0.176091259 + (double) binExp * 0.301029995663981);
    int B2, B5;
    int S2, S5;
    int M2, M5;
    int Bbits;
  ...
  public static FloatingDecimal readStringValue(String in)
    ...
    parseNumber: try {
      ...
      if (i < l && ((i != l - 1) || (in.charAt(i) != 'f' && in.charAt(i) != 'F'
  ...
  private static final int n5bits[] = {0, 3, 5, 7, 10, 12, 14, 17, 19, 21, 24, 26, 28, 31, 33, 35,
    ...
    if (!validInput) {
      ...
      if (exponent > DoubleConsts.MAX_EXPONENT) {
        ...
        if (exponent <= DoubleConsts.MAX_EXPONENT && exponent >= DoubleConsts.MIN_EXPONENT) {
          ...
          significand =
              ((((long) exponent + (long) DoubleConsts.EXP_BIAS) << (DoubleConsts.SIGNIFICAND_WIDTH
                  - 1)) & DoubleConsts.EXP_BIT_MASK) | (DoubleConsts.SIGNIF_BIT_MASK & significand);
