...
public class UniqueCombinationsCalculator {
  ...
  public static void main(String[] args) throws IOException, InvalidRowException {
    ...
    DataTypes[] valuesRead = fileReader.read();
    while (valuesRead != null) {
      MutableCharArrayString[] uniqueValues =
          new MutableCharArrayString[uniqueCombinationIndexes.length];
      for (int i = 0; i < uniqueCombinationIndexes.length; i++) {
        uniqueValues[i] = (MutableCharArrayString) valuesRead[uniqueCombinationIndexes[i]].clone();
      }
