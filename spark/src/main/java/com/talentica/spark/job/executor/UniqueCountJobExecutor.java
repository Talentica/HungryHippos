...
public class UniqueCountJobExecutor implements Serializable {
  ...
  public void startUniqueCountJob(JavaSparkContext context, HHRDDConfiguration hhrddConfiguration,
    ...
    JavaRDD<Tuple2<String, Long>> JavaRDD = javaPairRDD.mapPartitions(
        new FlatMapFunction<Iterator<Tuple2<String, Integer>>, Tuple2<String, Long>>() {
          ...
          public Iterator<Tuple2<String, Long>> call(Iterator<Tuple2<String, Integer>> t)
            ...
            for (Map.Entry<String, HyperLogLog> entry : hyperLogLogMap.entrySet()) {
              uniqueValues
                  .add(new Tuple2<String, Long>(entry.getKey(), entry.getValue().cardinality()));
            }
            return uniqueValues.iterator();
          }
        }, true);
    String fileDir =
        hhrddConfiguration.getOutputFile() + File.separatorChar + jobBroadcast.value().getJobId();
    new HHJavaRDD<Tuple2<String, Long>>(JavaRDD.rdd(), JavaRDD.classTag()).saveAsTextFile(fileDir);
